...
public:
    ...
    /**
     * @brief 读取端口号（范围校验在**窄化之前**完成）
     *
     * 为什么单独提供：`(uint16_t)GetUInt(key, def)` 会把 70000 静默截断成 4464
     * （70000 % 65536），而调用方的 `Validate()` 是在截断之后跑的，等于没校验 ——
     * 配置看起来加载成功，实际监听/连接在一个完全不同的端口上。
     * 这里先按 uint64 取原值，再判 (0, 65535]，失败时给出键名与实际值。
     */
    bool GetPort(const std::string& key, uint16_t def, uint16_t& out, std::string& err) const;
