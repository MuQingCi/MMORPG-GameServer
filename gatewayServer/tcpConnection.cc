...
TcpConnection::TcpConnection(EventLoop* loop, std::string name, 
                            ...
                            : loop_(loop), 
                              ...
                              // name_ 放最后：初始化列表顺序必须与 tcpConnection.h 的成员声明顺序一致
                              // （loop_, channel_, socket_, localAddr_, peerAddr_, name_）。
                              // 另注：channel_(loop, socket.Fd()) 读的是形参 socket，且 channel_ 声明在
                              // socket_ 之前、先被初始化，因此不会拿到 move 之后的 fd。
                              name_(name)
