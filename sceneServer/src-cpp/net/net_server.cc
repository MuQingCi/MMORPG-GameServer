...
// 握手体：服务间传输层信息，用固定小端布局（不需要 protobuf，也不允许被客户端伪造）
...
std::string PackHandshake(const HandshakeBody& h)
{
    std::string s;
...
bool UnpackHandshake(const std::string& body, HandshakeBody& out)
{
    if (body.size() < 9)
        return false;
    out.serviceId = static_cast<uint8_t>(body[0]);
    out.zoneId = GetU32(body.data() + 1);
    out.sceneId = GetU32(body.data() + 5);
    return true;
}
...
void NetServer::onPeerWritable(Conn& conn)
{
    ...
    const std::string body = PackHandshake(hs);
    Buffer frame;
    EncodeFrame(frame,
                MakeHeader(conn.linkType, Module::SYS, Method::SYS_HANDSHAKE, 0, 0,
...
void NetServer::onFrame(Conn& conn, const Header& head, const std::string& body)
{
    ...
    if (head.module == Module::SYS && head.method == Method::SYS_HANDSHAKE)
    {
